...
class VKCBot:
    ...
    def send(self,text,peer):
        peer = self.peer
        print(peer)
        msg = requests.post('{}messages.send'.format(self.vk_url), data=(('v', self.api_ver), ('random_id', '0'), ('access_token', self.token), ('peer_id', peer), ('message', text))).json()
        return msg
    ...
    def cmds(self,cmd,peer,msgid,message):
        filename = f'plugins/{cmd_list[cmd]}'
        print(filename)
                ...
                if 'updates' in res:
                    for updates in res['updates']:
                        if updates['type'] == 'message_new':
                            ...
                            if message[0].find(',') != -1:
                                message[0] = answ[0].replace(',','')
                            if message[0] in self.name and message[1] in cmd_list:
                                print('Упоминание в '+str(peer), message_text)
                                threading.Thread(target=self.cmds,args=(message[1],peer,msgid,message), daemon=True).start()
                            if message[0] in self.name and message[1] not in cmd_list:
                    	        send('Команда '+message[1]+' не найдена!', self.peer)
            except:
                data = self.get_lp()
